...
function validLlmResponse() {
  // Cumple las reglas del validator de real-tutor.ts:
  //  - Primer bloque: explanation (apertura)
  //  - Toda question → feedback adyacente siguiente
  //  - Último bloque: explanation (cierre consolidante)
  return {
    blocks: [
      ...
      { kind: "explanation", text: "Ya sabés que Argentina tiene 23 provincias y CABA." },
    ],
  };
}
...
describe("admin topics endpoints", () => {
  ...
  describe("POST /api/admin/topics/:id/generate", () => {
    ...
    it("happy path: persists generated_blocks + generated_at + model", async () => {
      ...
      expect(body.block_count).toBe(8);
